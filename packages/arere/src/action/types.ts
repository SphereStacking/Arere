...
export type {
  ...
  StepFormOptions,
  MergedFormResult,
} from '@/ui/prompts/form/types'

import type { FormFields, FormPage, FormResult, StepFormOptions } from '@/ui/prompts/form/types'
...
export interface TuiAPI {
  ...
  output: import('@/ui/output/types').OutputAPI
  /** Control API for timing and visual feedback */
  control: import('@/ui/control/types').ControlAPI
}
...
export interface ActionContext<TKeys extends string = string> {
   ...
   * ```
   */
  config: import('@/config/schema').ArereConfig
...
export interface Action<TKeys extends string = string> {
  ...
  pluginMeta?: import('@/plugin/types').PluginMeta
}
