...
#### Automatic Spinner/Progress Fallback

**Good news:** `spinner()` and `progress()` automatically work in headless mode! They return no-op implementations that do nothing, so you don't need to check `isInteractive()`:
...
export default defineAction({
  ...
  async run({ tui, $ }) {
    ...
    if (tui.control.isInteractive()) {
      ...
      // CI/CD environment: Custom output
