...
**Note:** In headless mode (CI/CD), spinner returns a no-op implementation that does nothing. No errors are thrown, and no visual output is shown.
...
**Note:** In headless mode (CI/CD), progress returns a no-op implementation that does nothing. No errors are thrown, and no visual output is shown.
...
In headless mode (CI/CD environments), `spinner()` and `progress()` automatically return **no-op implementations** that do nothing. This means you can safely use them without checking `isInteractive()`:
...
// In headless mode: no visual output, but no error either
```

If you need different behavior in each mode, check `isInteractive()`:
